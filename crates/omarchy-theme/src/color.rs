...
impl Rgba {
    ...
    pub fn parse(s: &str) -> Option<Rgba> {
        ...
        if let Some(rest) = s.strip_prefix("rgb(").or_else(|| s.strip_prefix("rgba(")) {
            ...
            let mut parts = rest.split(|c| c == ',' || c == ' ').filter(|p| !p.is_empty());
            ...
            return Some(Rgba { r: cl(r), g: cl(g), b: cl(b), a: cl(a) });
        }
        None
    }
    ...
    pub fn with_alpha(self, alpha: f32) -> Rgba {
        Rgba { a: (alpha.clamp(0.0, 1.0) * 255.0 + 0.5) as u8, ..self }
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_hex_forms() {
        assert_eq!(Rgba::parse("#1e1e2e"), Some(Rgba { r: 30, g: 30, b: 46, a: 255 }));
        assert_eq!(Rgba::parse("#fff"), Some(Rgba::WHITE));
        assert_eq!(Rgba::parse("#ff880080"), Some(Rgba { r: 255, g: 136, b: 0, a: 128 }));
        assert_eq!(Rgba::parse("#FFC94D"), Some(Rgba { r: 255, g: 201, b: 77, a: 255 }));
    ...
    fn parse_rgb_function() {
        assert_eq!(Rgba::parse("rgb(30, 30, 46)"), Some(Rgba { r: 30, g: 30, b: 46, a: 255 }));
        assert_eq!(Rgba::parse("rgba(0, 0, 0, 0.5)"), Some(Rgba { r: 0, g: 0, b: 0, a: 128 }));
    }

    #[test]
    fn u32_roundtrip() {
        let c = Rgba { r: 0x89, g: 0xb4, b: 0xfa, a: 0x80 };
