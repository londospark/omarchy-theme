...
impl Manifest {
    pub fn from_theme(theme: &Theme) -> Manifest {
        Manifest {
            ...
            font_family: theme.font.as_ref().map(|f| f.family.clone()).unwrap_or_default(),
            font_path: theme
                .font
                .as_ref()
...
pub fn resolved_palette(palette: &Palette) -> BTreeMap<String, String> {
    palette.entries().map(|(k, v)| (k.to_string(), v.to_string())).collect()
}
...
pub fn to_css(m: &Manifest) -> String {
    ...
    let s = &m.style;
    out.push_str(&format!("  --omarchy-normal-fill-alpha: {};\n", s.normal_fill_alpha));
    out.push_str(&format!("  --omarchy-hover-fill-alpha: {};\n", s.hover_fill_alpha));
    out.push_str(&format!("  --omarchy-focus-fill-alpha: {};\n", s.focus_fill_alpha));
    out.push_str(&format!("  --omarchy-selected-fill-alpha: {};\n", s.selected_fill_alpha));
    out.push_str(&format!("  --omarchy-pressed-fill-alpha: {};\n", s.pressed_fill_alpha));
    out.push_str(&format!("  --omarchy-spacing-scale: {};\n", s.spacing_scale));
    out.push_str(&format!("  --omarchy-font-base-size: {}px;\n", s.font_base_size));
    out.push_str("}\n");
...
pub fn to_toml(m: &Manifest) -> String {
    ...
    out.push_str("\n[palette]\n");
    for (k, v) in &m.palette {
        let value = if needs_quotes(v) { format!("\"{v}\"") } else { v.clone() };
        out.push_str(&format!("{k} = {value}\n"));
    }
    out.push_str("\n[style]\n");
    let s = &m.style;
    out.push_str(&format!("normal-fill-alpha = {}\n", s.normal_fill_alpha));
    out.push_str(&format!("normal-border-alpha = {}\n", s.normal_border_alpha));
    out.push_str(&format!("normal-border-width = {}\n", s.normal_border_width));
    ...
    out.push_str(&format!("selected-fill-alpha = {}\n", s.selected_fill_alpha));
    out.push_str(&format!("pressed-fill-alpha = {}\n", s.pressed_fill_alpha));
    out.push_str(&format!("selection-fill-alpha = {}\n", s.selection_fill_alpha));
...
pub fn to_c_header(m: &Manifest) -> String {
    let mut out = String::new();
    let guard = format!("OMARCHY_THEME_GENERATED_{}_H", m.theme.to_uppercase().replace('-', "_"));
    ...
    out.push_str(&format!("#define OMARCHY_THEME_MODE_{}\n", m.mode.to_uppercase()));
    out.push_str("\n/* palette (0xRRGGBBAA) */\n");
    for (k, v) in &m.palette {
        if let Some(c) = Rgba::parse(v) {
            let ident = k.to_uppercase();
            out.push_str(&format!("#define OMC_{ident} 0x{:08X}u /* {v} */\n", c.to_u32()));
        }
    }
    out.push_str("\n/* shell style tokens */\n");
    let s = &m.style;
    out.push_str(&format!("#define OMS_NORMAL_FILL_ALPHA {:?}f\n", s.normal_fill_alpha));
    out.push_str(&format!("#define OMS_HOVER_FILL_ALPHA {:?}f\n", s.hover_fill_alpha));
    out.push_str(&format!("#define OMS_SELECTED_FILL_ALPHA {:?}f\n", s.selected_fill_alpha));
    out.push_str(&format!("#define OMS_PRESSED_FILL_ALPHA {:?}f\n", s.pressed_fill_alpha));
    out.push_str(&format!("#define OMS_SPACING_SCALE {:?}f\n", s.spacing_scale));
    out.push_str(&format!("#define OMS_FONT_BASE_SIZE {:?}f\n", s.font_base_size));
    out.push_str("\n#endif\n");
    out
}
